function [VMSE]=Validating2()
clear;
clc;
load 'model2paras'
load 'themodel2'
beta=subsystemtwo.beta;
%% for traning data
[X,Y]=LSM_Data(yst,ust,optimcenter);
y_pre=X*beta;
error=Y-y_pre;
figure('Name','Subsystem two Training Result','NumberTitle','off');
subplot(2,1,1)
hold on
plot(Y,'k-')
plot(y_pre,'b--')
...
['Training MSE2 = ' num2str(MSE)]
%% for validating data
[X,Y]=LSM_Data(ysv,usv,optimcenter);
y_pre=X*beta;
error=Y-y_pre;
figure('Name','Subsystem two Validation Result','NumberTitle','off');
subplot(3,1,1)
hold on
plot(Y,'k-')
plot(y_pre,'b--')
